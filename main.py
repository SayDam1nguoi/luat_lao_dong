...
async def predict(data: Question):
    ...
    try:
        ...
        if handled and excel_payload:
            # excel_payload là JSON string -> parse sang dict
            try:
            ...
            # Build iz_list từ data[] (vì data đã có coordinates)
            iz_list = []
            ...
            map_intent = None
            # Nếu có tỉnh thì zoom theo tỉnh + show list điểm
            ...
            # Nếu TOÀN QUỐC, FE có thể fitBounds toàn bộ points nếu bạn muốn
            else:
                map_intent = {
                    "type": "points",
                    "iz_list": iz_list,
            ...
            return {
                "answer": excel_obj,          
                "type": "excel_query",
                "map_intent": map_intent,     # ✅ FE dùng để zoom/popup
                "requires_contact": False
            }
